...
def main():
    ...
    # pdf2png("marked.pdf", "marked")
    # marked_text = batch_OCR("marked")
    with open('marked_text.txt', 'r') as file:
        marked_text = file.read()
    ...
    plan_prompt = f"{HUMAN_PROMPT} First:\nPlease start by reading the assignment, what I submitted, and what I received back. Then, make a plan for where there might be potential to get marks back. It is very important to never mention feedback for me, only for the TA {AI_PROMPT}"
    ...
    initial_email_prompt = f"{HUMAN_PROMPT} You are a student who has recently been marked very harshly and unfairly. Draft an email that outlines your concerns and asks for consideration to get some marks back. You have made a plan and outlined areas where you can get marks back:\n {plan} \nONLY write the email, do not respond with anything else. Keep it professional but word it assertively. {AI_PROMPT}"
